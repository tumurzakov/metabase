...
(defn- format-userland-query-result
  "Make sure `query-result` `:status` is something other than `nil`or `:failed`, or return a failure response."
  ...
  (fn [{{:keys [userland-query?]} :middleware, :as query} respond raise canceled-chan]
    (if-not userland-query?
      ...
      (let [query (assoc-in query [:info :query-hash] (qputil/query-hash query))]
        (qp
         query
         (comp respond (partial format-userland-query-result (query-execution-info query)))
         raise
         canceled-chan)))))
